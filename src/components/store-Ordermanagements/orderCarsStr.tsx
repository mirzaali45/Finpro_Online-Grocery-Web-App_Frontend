...
const OrderCardStr: React.FC<OrderCardProps> = ({ order }) => {
  return (
    <div className="bg-gray-700 p-4 rounded-lg shadow-md hover:shadow-xl transition duration-300">
      <h2 className="text-xl font-semibold text-blue-400">Order #{order.order_id}</h2>
      ...
      <p className="text-white">Shipping Status: {order.Shipping[0]?.shipping_status}</p>
    </div>
  );
};
