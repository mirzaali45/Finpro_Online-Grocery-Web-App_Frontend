...
export const VoucherList: React.FC = () => {
  ...
  const handleRedeemVoucher = async (voucher: Voucher) => {
    try {
      await voucherService.redeemVoucher(voucher.voucher_id);
      setVouchers((prevVouchers) =>
        prevVouchers.map((v) =>
          v.voucher_id === voucher.voucher_id ? { ...v, is_redeemed: true } : v
        )
      );
      toast.success(`Voucher ${voucher.voucher_code} redeemed successfully!`);
    } catch (err) {
      console.error("Voucher redemption failed");
