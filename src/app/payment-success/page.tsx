...
export default function PaymentSuccessPage() {
    ...
    try {
      ...
      if (response && response.data) {
        ...
        if (orderDetails) {
          ...
          // If order is still in awaiting_payment status, automatically trigger status update
          if (orderDetails.status === "awaiting_payment") {
            console.log(
              `⚠️ Order ${orderId} is still in awaiting_payment status after successful payment redirect`
            );

            // Automatically update order status to "shipped" if payment was successful
  ...
  // Function to automatically update order status to "shipped"
  const updateOrderStatusToShipped = async (orderId: string) => {
    try {
      const token = await getAuthToken();
      if (!token) {
        toast.error("Authentication error");
      ...
      const response = await orderService.updateOrderStatus(token, orderId, "shipped");

      if (response.success) {
        toast.success("Order status updated to shipped");
        fetchOrderDetails(); // Fetch the updated order details after status change
      } else {
